...
# %%
def add_five(arr):
    return np.add(arr,5*np.ones(arr.shape, dtype=np.uint8)) # 👈 Insérez le code ici
...
# %%
def square(arr):
    return np.multiply(arr,arr)  # 👈 Insérez le code ici
...
def sin_values():
    ...
    return np.sin(tab)  # 👈 Insérez le code
...
def g_vectorized(x):
    tab1=np.copy(x)
    x[x<=0]=1
    return np.multiply(tab1,x)  # 👈 Insérez le code ici
